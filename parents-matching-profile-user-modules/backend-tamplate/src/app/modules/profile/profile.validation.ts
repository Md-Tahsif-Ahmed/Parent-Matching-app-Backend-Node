...
const simpleKV = z.object({
  typeName: z.string().min(1, 'typeName required').optional(),
...
export const ProfileValidation = {
  updateBasic: z.object({
    body: z.object({
      ...
      childAge: z.number().int().min(0).max(600).optional(), // 0..600 months
    }),
  }),

  setJourney: z.object({
    body: z.object({
      journeyName: z.string().trim().min(1, 'journey name required'),
    }),
  }),
  ...
  addPhoto: z.object({}),            // using multipart – validated by file presence
