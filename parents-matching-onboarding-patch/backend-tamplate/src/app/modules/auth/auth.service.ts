...
const forgetPasswordToDB = async (email: string) => {
  ...
  const authentication = {
    oneTimeCode: otp,
    expireAt: new Date(Date.now() + (config.auth.otpExpireMin || 5) * 60000),
  };
...
const verifyEmailToDB = async (payload: IVerifyEmail) => {
  ...
  await ResetToken.create({
    user: isExistUser._id,
    token: createToken,
    expireAt: new Date(Date.now() + (config.auth.resetTokenExpireMin || 10) * 60000),
  });
  return {
...
const resendVerificationEmailToDB = async (email: string) => {
  ...
  await User.findByIdAndUpdate(existingUser._id, {
    authentication: { oneTimeCode: otp, expireAt: new Date(Date.now() + (config.auth.otpExpireMin || 5) * 60000) },
  });
